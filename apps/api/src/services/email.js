...
/**
 * Resolve the SMTP provider for a given organization.
 * DB settings take priority over environment variables.
 */
async function getProvider(organizationId) {
  if (organizationId) {
...
async function logEmail({ organizationId, to, subject, template, status, error }) {
  try {
    await query(
      `INSERT INTO email_log (id, organization_id, to_address, subject, template, status, error, sent_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [genId(), organizationId || null, to, subject, template || null, status, error || null, Date.now()]
    );
  } catch (e) {
 ...
 * @param {{ organizationId?, to, subject, html, text, template? }} opts
 */
export function sendEmail({ organizationId, to, subject, html, text, template }) {
  getProvider(organizationId).then(provider =>
    provider.send(to, subject, html, text)
  ).then(() => {
    logEmail({ organizationId, to, subject, template, status: 'sent' });
  }).catch(err => {
    console.error('[email] send failed:', err.message);
    logEmail({ organizationId, to, subject, template, status: 'failed', error: err.message });
  });
}

// ── Templates ─────────────────────────────────────────────────────────────────

export function sendInviteEmail({ organizationId, to, studioName, galleryTitle, inviteUrl }) {
  ...
  sendEmail({ organizationId, to, subject, html, text, template: 'invite' });
}

export function sendMagicLinkEmail({ organizationId, to, magicUrl }) {
  ...
  sendEmail({ organizationId, to, subject, html, text, template: 'magic-link' });
}

export function sendPhotosReadyEmail({ organizationId, to, photographerName, galleryTitle, galleryAdminUrl }) {
  ...
  sendEmail({ organizationId, to, subject, html, text, template: 'photos-ready' });
}

export function sendGalleryReadyEmail({ organizationId, to, galleryTitle, galleryUrl }) {
  ...
  sendEmail({ organizationId, to, subject, html, text, template: 'gallery-ready' });
}

export function sendAccessResendEmail({ organizationId, to, galleryTitle, managementUrl }) {
  ...
  sendEmail({ organizationId, to, subject, html, text, template: 'access-resend' });
}
